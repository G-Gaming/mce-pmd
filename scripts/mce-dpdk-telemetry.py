...
def read_socket(sock, buf_len, echo=True, pretty=True):
...
def map_input(input_v):
    ...
    if items[0] == '/':
        return input_v
    if items[0] == "md" or items[0] == "rd":
        items[0] = "/mce/reg_read"
        return ",".join(items[:2]) + " " + " ".join(items[2:])
    elif items[0] == "mw" or items[0] == "mr":
        items[0] = "/mce/reg_write"
        return ",".join(items[:2]) + " " + " ".join(items[2:])
    elif items[0] == "link":
        port_id = "0"
        if len(items) >= 2:
            port_id = items[1]
        return "/mce/dump,{} link".format(port_id)
    elif items[0] == "version":
        port_id = "0"
        if len(items) >= 2:
            port_id = items[1]
        return "/mce/dump,{} version".format(port_id)
    elif items[0] == "port":
        port_id = "0"
        if len(items) >= 2:
            port_id = items[1]
        return "/mce/dump,{} port".format(port_id)
    elif items[0] == "sfp-info":
        port_id = "0"
        if len(items) >= 2:
            port_id = items[1]
        return "/mce/dump,{} sfp-info".format(port_id)
    else:
        return "/mce/" +input_v

def adj_input(input_v):
    items = input_v.split(" ")
    if len(items) <= 1:
        return input_v
    else:
...
def handle_socket(args, path):
    ...
    # interactive prompt
    try:
        if len(cmdline_cmd) > 0:
            text = cmdline_cmd
            cmdline_cmd = ""
        else:
            text = input(prompt).strip()
        while text != "quit" and text != "q":
            text = map_input(text)
            if text.startswith('/'):
                text = adj_input(text)
...
parser.add_argument('-c', '--cmd', action="store", default="", help='cmdline')
args = parser.parse_args()
if args.list:
    list_fp()
    sys.exit(0)
if len(args.cmd) > 0:
    cmdline_cmd = args.cmd
